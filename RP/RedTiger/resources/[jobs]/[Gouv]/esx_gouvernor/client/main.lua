	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'citizen_wear' then
				...
				TriggerServerEvent('esx_gouverneur:removeWeapon','WEAPON_SMG')
			...
			if data.current.value == 'gouvernor_wear' then
				...
				TriggerServerEvent('esx_gouverneur:giveWeapon','WEAPON_SMG', 450)
...
function OpenHelicoSpawnerMenu()
	...
	local elements = {
		{label = 'Sortir H�lico', value = 'vehicle_list'},
	}
		...
		function(data, menu)
			if data.current.value == 'vehicle_list' then
				...
				if Config.EnableSocietyOwnedVehicles then
						...
						local elements = {
							{label = 'Petit H�lico', value = 'Buzzard2'},
							{label = 'Moyen h�lico', value = 'Frogger2'}
		...
		function(data, menu)
			if data.current.value == 'vehicle_list' then
				...
				if Config.EnableSocietyOwnedVehicles then
						...
						local elements = {
							{label = 'Voiture personnelle', value = 'Cog552'},
							{label = 'SUV', value = 'xls2'}
						}
...
AddEventHandler("esx_gouverneur:sendRequest", function(name,id)
	stopRequest = true
	SendNotification("~b~"..name.." ~w~a sonn� � la porte du gouvernement.")
