...
AddEventHandler('playerConnecting', function() 
    ...
    sendToDiscord("Entre sur le Serveur", "**" .. GetPlayerName(source) .. "** c'est connecter au serveur.", 65280)
end)

AddEventHandler('playerDropped', function(reason) 
	local color = 16711680
	if string.match(reason, "Kick") or string.match(reason, "Bannis") then
		color = 16007897
	end
  sendToDiscord("Parti du Serveur", "**" .. GetPlayerName(source) .. "** est parti du serveur. \n Raison: " .. reason, color)
end)

RegisterServerEvent('playerDied')
AddEventHandler('playerDied',function(message)
    sendToDiscord("Morts", message, 16711680)
end)
